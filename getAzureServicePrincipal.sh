...
DEFAULT_ACCOUNT_ID=$(printf %s "$DEFAULT_ACCOUNT" | cut -f3)
if [ ! -z "$DEFAULT_ACCOUNT_ID" ]; then
    export DEFAULT_ACCOUNT_NAME=`printf %s "$DEFAULT_ACCOUNT" | cut -f6`
    echo "Current subscription (default): \"${DEFAULT_ACCOUNT_NAME}\" (${DEFAULT_ACCOUNT_ID})"
    echo ""
    export ACCOUNT_LIST=`az account list -o tsv`
    export ACCOUNT_LIST_ID=`printf %s "$ACCOUNT_LIST" |  cut -f3`
    export ACCOUNT_LIST_NAMES=`echo $ACCOUNT_LIST |  cut -f6 -d$' '`
    ...
    for line in $(printf %s "$ACCOUNT_LIST"); do
        echo "${COUNT}) $(printf %s "$line" | cut -f6 ) || ($(echo $line | cut -f3 ))"
        ((COUNT++))
    done
    set +f
    unset IFS
